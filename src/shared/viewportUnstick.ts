...
//
// NOTE on the iOS 26 floating-footer variant (share-sheet / Add-to-Home-Screen trigger):
// researched + closed 2026-08-06 as an iOS 26 WebKit regression (WebKit #297779 family) -
// the corrupted fixed-position anchoring lives in the NATIVE layer while every JS-readable
// metric reads healthy, so no in-page fix exists; Apple is patching it in Safari/iOS 26.x.
// The staging flight recorder that diagnosed it was removed the same day. Do not re-add
// in-page workarounds for that variant - two metric-driven fixes were field-tested and
// bounced off (details in the project memory).
